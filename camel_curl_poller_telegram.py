...
def notify_new(new_deals):
    """
    Send Telegram messages for new deals instead of macOS notifications.
    """

    api_url = f"https://api.telegram.org/bot{const.BOT_TOKEN}/sendMessage"

    for deal in new_deals[:5]:
        text = (
            "🛒 New CamelCamelCamel deal\n"
            f"{deal['name']} (-{deal['pct']}%)\n"
            f"{deal['amazon_url']}"
        )
        try:
            resp = requests.post(
                api_url,
                data={"chat_id": const.CHAT_ID, "text": text},
                timeout=10,
            )
