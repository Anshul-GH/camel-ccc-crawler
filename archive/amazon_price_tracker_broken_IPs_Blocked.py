...
def fetch_html(url: str, retry_count: int = 0) -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"Fetch fail {retry_count+1}/3 {url}")
        if retry_count < 3:
            return fetch_html(url, retry_count + 1)
        logger.info(f"Max retries exceeded: {url}")
        return None
...
async def main() -> None:
    ...
    logger.info("🚀 Amazon-only Price Tracker (RPi, requests+BS4) started!")
    
    while True:
        shuffled_items = items.copy()
        random.shuffle(shuffled_items)
        logger.info(f"Shuffled cycle: {len(shuffled_items)} items")
        ...
        summary_msg = f"✅ Checked {active_items} URLs @ {timestamp}"
        await send_telegram(summary_msg)
        
        save_state(STATE_FILE, state)
        logger.info(f"Sleeping {POLL_INTERVAL/60:.0f}min...")
